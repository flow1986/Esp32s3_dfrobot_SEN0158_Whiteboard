...
static inline double mapd(double v, double in_min, double in_max, double out_min, double out_max) {
  ...
  if (std::fabs(in_max - in_min) < 1e-12) {
    // Wenn Eingangsbereich null ist, liefere out_min (konservativ).
    return out_min;
  }
...
void calculateHomography() {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (!std::isfinite(X) || !std::isfinite(Y) || !std::isfinite(x) || !std::isfinite(y)) {
      // Ungültige Werte -> Fallback auf Identität
      for (int k = 0; k < 9; ++k) cfg.H[k] = (k % 4 == 0) ? 1.0 : 0.0;
      cfg_save();
      return;
    }
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (best < 1e-12) {
      // Singuläre/fast-singuläre Matrix -> setze Homographie auf Identität als sicherer Fallback
      for (int k = 0; k < 9; ++k) cfg.H[k] = (k % 4 == 0) ? 1.0 : 0.0;
      cfg_save();
      return;
    }
    ...
    for (int j = i + 1; j < 8; j++) {
      double denom = a[i][i];
      if (std::fabs(denom) < 1e-18) continue; // zusätzlicher Schutz
      double f = a[j][i] / denom;
      b[j] -= f * b[i];
  ...
  for (int i = 7; i >= 0; i--) {
    ...
    if (std::fabs(diag) < 1e-12) {
      // sehr kleines Diagonalelement: setze Lösungselement sicher auf 0
      cfg.H[i] = 0.0;
      continue;
    }
    double val = b[i];
    for (int j = i + 1; j < 8; j++) val -= a[i][j] * cfg.H[j];
    cfg.H[i] = val / diag;
    if (!std::isfinite(cfg.H[i])) cfg.H[i] = 0.0;
  }
  cfg.H[8] = 1.0;
  cfg_save();
}
...
void setup() {
  ...
  server.on("/exp", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    if (cfg.manualExposure) { sensor_t * s = esp_camera_sensor_get(); if (s) s->set_aec_value(s, cfg.exposureValue); }
    cfg_save();
    request->send(200);
  });
...
void loop() {
  ...
  if (found) {
    ...
    if (calibStep >= 0 && calibStep < 5) {
      ...
      if (!std::isfinite(z) || std::fabs(z) < 1e-9) z = (z >= 0 ? 1e-9 : -1e-9);
      ...
      // 11) HID-Ausgabe
      double screenW = (cfg.screenW > 0) ? (double)cfg.screenW : 1.0;
      double screenH = (cfg.screenH > 0) ? (double)cfg.screenH : 1.0;
      int16_t hx = (int16_t)constrain(lround(mapd(outx, 0.0, screenW, 0.0, 32767.0)), 0, 32767);
      int16_t hy = (int16_t)constrain(lround(mapd(outy, 0.0, screenH, 0.0, 32767.0)), 0, 32767);
